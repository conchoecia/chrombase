...
| --- | --- | --- |
| `bioproject` | The `<Publication>` element of the assembly's BioProject record | The submitter's own assertion, and the only route that identifies the assembly directly. Not automatically right, though: some projects carry an old mitogenome paper or a consortium overview rather than a description of the assembly, so the link is only labelled `authoritative` if its date is consistent with the assembly's release. Note that E-utilities `id=` takes a numeric UID, not an accession — passing `PRJEB90089` silently returns UID 90089, an unrelated project — so accessions are resolved through esearch and each record is checked against the accession it reports as its own. |
...
A publication that predates the assembly's release, or trails it by years, is
also disqualified from the `authoritative` label outright, however well it
scores otherwise: it cannot be a description of the assembly in hand.
...
BioProject lookups are batched — esearch takes a disjunction of accessions and
efetch takes many UIDs at once, so a chunk of 100 costs two requests rather than
two hundred. Doing them one at a time is enough traffic to get the client
rate-limited for minutes, and every refusal costs a publication link. A full run
over several thousand assemblies takes a couple of hours, nearly all of it in
the per-assembly Europe PMC queries.

Two runs of the script must not share a cache directory: both hold the cache in
memory and write it whole, so the second to finish discards the other's work.
